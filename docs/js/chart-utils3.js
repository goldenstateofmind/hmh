...
var xScaleLinear = d3.scaleLinear().range([0, width]).nice()
var yScaleLinear = d3.scaleLinear().range([height, 0]).nice()
...
function initChart(bandCount) {
  ...
  svgG.append('path').attr('id', 'data-a-area').attr('class', 'area')

  svgG.append('path').attr('id', 'data-b-area').attr('class', 'area')

  svgG.append('path').attr('id', 'data-a-line').attr('class', 'line')

  svgG.append('path').attr('id', 'data-b-line').attr('class', 'line')
  ...
  svg
    ...
    .attr('transform', (d) => {
      return `translate(${
        margin.left + indent + A.bandwidthFraction * xScaleBand.bandwidth() + 4
      }, ${margin.top})`
    })
  ...
  d3.select('#div-for-svg-chart')
    ...
    .on('mouseover', (d) => {
      ...
      if (d === 'chart-bar') {
        d3.selectAll('#data-a-stacked-bars,#data-b-stacked-bars')
          ...
          .duration(400)
          .style('opacity', 1)
        d3.select('#legend').transition().duration(400).style('opacity', 1)
        d3.select('#hmh-g-area-line-chart')
          .transition()
          .duration(400)
          .style('opacity', 0)
        d3.select('#toltp').transition().duration(400).style('opacity', 1)
        ...
        d3.selectAll('#data-a-stacked-bars,#data-b-stacked-bars')
          ...
          .duration(400)
          .style('opacity', 0)
        d3.select('#legend').transition().duration(400).style('opacity', 0)
        d3.select('#hmh-g-area-line-chart')
          .transition()
          .duration(400)
          .style('opacity', 1)
        d3.select('#toltp').transition().duration(400).style('opacity', 0)
    ...
    .append('span')
    .attr('class', 'inline-flex m-1 underline')
    .html((d) => (d === 'chart-bar' ? 'bars' : 'lines'))
}
...
function updateChartArea(aData, aData_hi, aData_lo, bData, bData_hi, bData_lo) {
  ...
  var yMax = Math.max(...aData.concat(bData).concat(aData_hi).concat(bData_hi))
...
createLegend = (svg) => {
  ...
  const g = svg
    ...
    .data(Object.values(A.colorLookup).slice().reverse(), (d) => {
      // return d;
...
function updateChart2(aData, bData) {
  ...
  const heightLinear = (d) => {
    const diff = yScaleLinear(d[0]) - yScaleLinear(d[1])
  ...
  const enterBars = bars
    ...
    .attr('y', (d) => yScaleLinear(d[1]))
    .attr('height', heightLinear)
